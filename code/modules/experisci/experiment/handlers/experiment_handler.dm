...
/datum/component/experiment_handler/proc/get_available_servers(turf/turf_to_check_for_servers = null)
	...
	if (!turf_to_check_for_servers.virtual_level_trait(ZTRAIT_STATION))
	...
	for (var/obj/machinery/rnd/server/server in SSresearch.servers)
		...
		if (position_of_this_server_machine && position_of_this_server_machine.virtual_level_trait(ZTRAIT_STATION))
