...
/obj/effect/pod_landingzone/proc/setupSmoke(rotation)
	...
	for ( var/i in 1 to length(smoke_effects))
		...
		addtimer(CALLBACK(smoke_part, /obj/effect/supplypod_TYPE_PROC_REF(smoke,drawSelf), i), time, TIMER_CLIENT_TIME) //Go onto the last step after a very short falling animation
