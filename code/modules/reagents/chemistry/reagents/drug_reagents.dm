...
/datum/reagent/drug/tetrahydrocannabinol
	name = "THC"
	description = "A mild psychoactive drug from the Hemp plant used for recreational purposes."
	color = "#ae8300"
...
/datum/reagent/drug/tetrahydrocannabinol/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
...
/datum/reagent/drug/tetrahydrocannabiphorol
	name = "THCP"
	description = "An intense psychoactive drug from the Hemp plant used for recreational purposes."
	color = "#85690c"
	overdose_threshold = INFINITY
	ph = 6
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/drug/tetrahydrocannabiphorol/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	M.apply_status_effect(/datum/status_effect/stoned)
	if(isturf(M.loc) && !isspaceturf(M.loc) && !HAS_TRAIT(M, TRAIT_IMMOBILIZED) && DT_PROB(5, delta_time))
		step(M, pick(GLOB.cardinals))
	if(DT_PROB(1, delta_time))
		...
		to_chat(M, SPAN_NOTICE(smoke_message))
	if(DT_PROB(2, delta_time))
		M.emote(pick("smile","laugh","twitch","drool","moan","giggle"))
	M.adjust_nutrition(-2 * REM * delta_time) //munchies
	if(DT_PROB(5.8, delta_time) && M.body_position == LYING_DOWN && !M.IsSleeping()) //chance to fall asleep if lying down
		to_chat(M, SPAN_WARNING("You doze off..."))
		M.Sleeping(15 SECONDS)
	if(DT_PROB(5.8, delta_time) && M.buckled && M.body_position != LYING_DOWN && !M.IsParalyzed()) //chance to be couchlocked if sitting
		to_chat(M, SPAN_WARNING("It's too comfy to move..."))
		M.Paralyze(15 SECONDS)
	return ..()
