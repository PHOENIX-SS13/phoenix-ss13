...
/datum/action/vehicle/sealed/mecha/mech_toggle_internals/Trigger()
	...
	chassis.balloon_alert(owner, "taking air from [chassis.use_internal_tank ? "internal airtank" : "environment"]", "[icon2html(chassis, owner)]<span class='notice'>Now taking air from [chassis.use_internal_tank?"internal airtank":"environment"].</span>")
...
/datum/action/vehicle/sealed/mecha/mech_cycle_equip/Trigger()
	...
	if(available_equipment.len == 0)
		chassis.balloon_alert(owner, "no equipment available", "[icon2html(chassis, owner)]<span class='warning'>No equipment available!</span>")
		return
	if(!chassis.selected)
		chassis.selected = available_equipment[1]
		chassis.balloon_alert(owner, "[chassis.selected] selected", "[icon2html(chassis, owner)]<span class='notice'>You select [chassis.selected].</span>")
	...
	for(var/equipment in available_equipment)
		...
		if(available_equipment.len == number)
			...
			chassis.balloon_alert(owner, "switched to no equipment", "[icon2html(chassis, owner)]<span class='notice'>You switch to no equipment.</span>")
			button_icon_state = "mech_cycle_equip_off"
		else
			chassis.selected = available_equipment[number+1]
			chassis.balloon_alert(owner, "switched to [chassis.selected]", "[icon2html(chassis, owner)]<span class='notice'>You switch to [chassis.selected].</span>")
			button_icon_state = "mech_cycle_equip_on"
		send_byjax(chassis.occupants,"exosuit.browser","eq_list",chassis.get_equipment_list())
		UpdateButtonIcon()
		return
...
/datum/action/vehicle/sealed/mecha/mech_toggle_lights/Trigger()
	...
	if(!(chassis.mecha_flags & HAS_LIGHTS))
		chassis.balloon_alert(owner, "the mech lights are broken!", "<span class='warning'>This mechs lights are destroyed!</span>")
		return
	...
	chassis.balloon_alert(owner, "toggled lights [chassis.mecha_flags & LIGHTS_ON ? "on":"off"]", "[icon2html(chassis, owner)]<span class='notice'>Toggled lights [(chassis.mecha_flags & LIGHTS_ON)?"on":"off"].</span>")
...
/datum/action/vehicle/sealed/mecha/mech_overload_mode/Trigger(forced_state = null)
	...
	if(chassis.leg_overload_mode)
		...
		chassis.balloon_alert(owner,"leg actuators overloaded", "[icon2html(chassis, owner)]<span class='danger'>You enable leg actuators overload.</span>")
	else
		chassis.movedelay = initial(chassis.movedelay)
		chassis.step_energy_drain = chassis.normal_step_energy_drain
		chassis.balloon_alert(owner, "you disable the overload", "[icon2html(chassis, owner)]<span class='notice'>You disable leg actuators overload.</span>")
	UpdateButtonIcon()
...
/datum/action/vehicle/sealed/mecha/mech_switch_damtype/Trigger()
	...
	switch(chassis.damtype)
		if("tox")
			new_damtype = "brute"
			chassis.balloon_alert(owner, "your punches will now deal brute damage", "[icon2html(chassis, owner)]<span class='notice'>Your exosuit's hands form into fists.</span>")
		if("brute")
			new_damtype = "fire"
			chassis.balloon_alert(owner, "your punches will now deal burn damage", "[icon2html(chassis, owner)]<span class='notice'>A torch tip extends from your exosuit's hand, glowing red.</span>")
		if("fire")
			new_damtype = "tox"
			chassis.balloon_alert(owner,"your punches will now deal toxin damage", "[icon2html(chassis, owner)]<span class='notice'>A bone-chillingly thick plasteel needle protracts from the exosuit's palm.</span>")
...
/datum/action/vehicle/sealed/mecha/mech_toggle_phasing/Trigger()
	...
	chassis.balloon_alert(owner, "[chassis.phasing ? "enabled" : "disabled"] phasing", "[icon2html(chassis, owner)]<font color=\"[chassis.phasing?"#00f\">En":"#f00\">Dis"]abled phasing.</font>")
	UpdateButtonIcon()
