SUBSYSTEM_DEF(gamemode)
	...
	var/voted_storyteller = /datum/storyteller/ghost
...
/datum/controller/subsystem/gamemode/proc/admin_panel(mob/user)
	...
	switch(panel_page)
		if(GAMEMODE_PANEL_VARIABLES)
			...
			dat += "<HR><b>Allow Storyteller Pop Scaling:</b> <a href='byond://?src=[REF(src)];panel=main;action=vars;var=allow_pop_scaling'>[allow_pop_scaling ? "Yes" : "No"]</a>"
			...
			for(var/track in event_tracks)
				dat += "<BR>[track]: <a href='byond://?src=[REF(src)];panel=main;action=vars;var=pop_scale_threshold;track=[track]'>[pop_scale_thresholds[track]]</a>"
			...
			for(var/track in event_tracks)
				dat += "<BR>[track]: <a href='byond://?src=[REF(src)];panel=main;action=vars;var=pop_scale_penalty;track=[track]'>[pop_scale_penalties[track]]%</a>"
