...
/obj/item/toy/mecha/proc/combat_sleep(delay, obj/item/toy/mecha/attacker, mob/living/carbon/attacker_controller, mob/living/carbon/opponent)
	...
	if(!attacker) //if there's no attacker, then attacker_controller IS the attacker
		...
		if(!in_range(src, attacker)) //and the two toys aren't next to each other, the battle ends
			attacker_controller.visible_message(
		...
		if(!in_range(attacker, attacker_controller) && !(attacker_controller.dna.check_mutation(TK)))
			attacker_controller.visible_message(
				SPAN_NOTICE("[attacker_controller.name] seperates from [attacker], ending the battle."),
		...
		if(opponent)
			...
			if(!in_range(src, opponent) && !(opponent.dna.check_mutation(TK)))
				opponent.visible_message(
					SPAN_NOTICE("[opponent.name] seperates from [src], ending the battle."),
		...
		//if it's not PVP and the attacker_controller isn't next to the defending toy (and doesn't have telekinesis), the battle ends
			...
			if (!in_range(src, attacker_controller) && !(attacker_controller.dna.check_mutation(TK)))
				attacker_controller.visible_message(
					SPAN_NOTICE("[attacker_controller.name] seperates from [src] and [attacker], ending the battle."),
...
/obj/item/toy/mecha/proc/mecha_brawl(obj/item/toy/mecha/attacker, mob/living/carbon/attacker_controller, mob/living/carbon/opponent)
	...
	attacker_controller.visible_message(
		SPAN_DANGER("[attacker_controller.name] collides [attacker] with [src]! Looks like they're preparing for a brawl!"),
	...
	while(combat_health > 0 && attacker.combat_health > 0 && battle_length < MAX_BATTLE_LENGTH)
		...
		if(special_attack_charged)
			src_controller.visible_message(
				SPAN_DANGER("[src] unleashes its special attack!!"),
			...
			attacker_controller.visible_message(
				SPAN_DANGER("[attacker] unleashes its special attack!!"),
			...
			switch(rand(1,8))
				if(1 to 3) //attacker wins
					if(attacker.special_attack_cooldown == 0 && attacker.combat_health <= round(attacker.max_combat_health/3)) //if health is less than 1/3 and special off CD, use it
						...
						attacker_controller.visible_message(
							SPAN_DANGER("[attacker] begins charging its special attack!!"),
						...
						combat_health--
						attacker_controller.visible_message(
							SPAN_DANGER("[attacker] devastates [src]!"),
						...
						if(prob(5))
							...
							attacker_controller.visible_message(
								SPAN_BOLDWARNING("...and lands a CRIPPLING BLOW!"),
								SPAN_BOLDWARNING("...and you land a CRIPPLING blow on [src]!"),
								null,
								COMBAT_MESSAGE_RANGE
							)
				...
				if(6 to 8) //defender wins
					if(special_attack_cooldown == 0 && combat_health <= round(max_combat_health/3)) //if health is less than 1/3 and special off CD, use it
						...
						src_controller.visible_message(
							SPAN_DANGER("[src] begins charging its special attack!!"),
						...
						attacker.combat_health--
						src_controller.visible_message(
							SPAN_DANGER("[src] smashes [attacker]!"),
						...
						if(prob(5))
							...
							src_controller.visible_message(
								SPAN_BOLDWARNING("...and lands a CRIPPLING BLOW!"), \
								SPAN_BOLDWARNING("...and you land a CRIPPLING blow on [attacker]!"),
								null,
								COMBAT_MESSAGE_RANGE
							)
				else
					attacker_controller.visible_message(
						SPAN_NOTICE("[src] and [attacker] stand around awkwardly."),
	...
	var/list/winlines = list(
		"YOU'RE NOTHING BUT SCRAP!",
		"I'LL YIELD TO NONE!",
		"GLORY IS MINE!",
		"AN EASY FIGHT.",
		"YOU SHOULD HAVE NEVER FACED ME.",
		"ROCKED AND SOCKED.",
	)
	...
	if(attacker.combat_health <= 0 && combat_health <= 0) //both lose
		...
		attacker_controller.visible_message(
			SPAN_BOLDNOTICE("MUTUALLY ASSURED DESTRUCTION!! [src] and [attacker] both end up destroyed!"),
		...
		attacker_controller.visible_message(
			SPAN_NOTICE("[attacker] falls apart!"),
			SPAN_NOTICE("[attacker] falls apart!"),
			null,
			COMBAT_MESSAGE_RANGE
		)
		say("[pick(winlines)]")
		src_controller.visible_message(
			SPAN_NOTICE("[src] destroys [attacker] and walks away victorious!"),
		...
		attacker_controller.visible_message(
			SPAN_NOTICE("[attacker] demolishes [src] and walks away victorious!"),
